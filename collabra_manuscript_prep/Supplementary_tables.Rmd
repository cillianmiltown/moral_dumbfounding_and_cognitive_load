...
source("../set_up_manipulation_check.R")
...
df3 <- x
test <- as.data.frame.matrix((table(df3$InCS,df3$correct)))

`rownames<-`(test, c("nothing wrong","no reason","reasons"))
test <- as.matrix((test))
test1 <- cbind(test[,1],test[,2])
test2 <- `colnames<-`(test1, c("Correct","Incorrect"))

tb_count_perc <- function(x){
  tc <- table(x$InCS,x$correct)
  cbind(tc[,1],paste0(round(tc[,1]/sum(x$correct=="Correct")*100),"%"),
        tc[,2],paste0(round(tc[,2]/sum(x$correct=="Incorrect")*100),"%"))
  }
...
makespanners <- function(){
  ...
  control <- c(4,5)
  spans <- list(cog_load,control)
  names(spans) <- c("Correct","Incorrect")
  spans
}
...
```{r tabS6EngagementAndDumbfounding,results = 'asis', include=TRUE}


apa_table(
   test
   , align = c("l", "c", "c", "c", "c")
   , caption = "Study 6 – Response to the critical slide depending on performance in cognitive load task"
   #, added_stub_head = "Response to critical slide"
   , col_spanners = makespanners()
   #, note = "* = sig. at \\emph{p} < .05; ** = sig. at \\emph{p} < .001"
   , escape = FALSE
